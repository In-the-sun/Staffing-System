...
class DataBaseManager(object):
    def __init__(self):
        """
        你需要在这里初始化MongoDB的连接，连上本地MongoDB，库名为workforce，集合名为people_info
        """
        client = MongoClient("192.168.222.151", 27017)
        database = client.workforce
    ...
    def search_info(self, search_terms):
        """
        你需要实现这个方法，根据多个字段筛选符合条件的人员信息。
        search_terms 是一个字典，包含需要筛选的字段和对应的值。
        :param search_terms: 格式为 {'字段名1': '值1', '字段名2': '值2', ...}
        :return: 匹配的人员信息列表，每个人员信息是一个字典
        """
        query = {'deleted': 0}  # 默认查询条件为未删除的人员信息
        # 构建查询条件
        for field, value in search_terms.items():
            # 进行模糊匹配查询
            if field == 'name':
                query[field] = {'$regex': value, '$options': 'i'}
            elif field == 'origin_home':
                query[field] = {'$regex': value, '$options': 'i'}
            elif field == 'current_home':
                query[field] = {'$regex': value, '$options': 'i'}
            else:
                # 对其他字段进行精确匹配查询
                query[field] = value
        # 查询符合条件的人员信息
        info_list = list(self.handler.find(query, {'_id': 0}))
        return info_list
